...
describe('HooksService status read/write', () => {
  function makeHooksService() {
    ...
    const svc = new HooksService({statusDir});
    return {svc, statusDir};
  }

  test('readStatus returns null when file does not exist', () => {
    const {svc} = makeHooksService();
    expect(svc.readStatus('dev-nonexistent-session')).toBeNull();
  });

  test('readStatus returns data for a fresh status file', () => {
    const {svc, statusDir} = makeHooksService();
    const sessionName = 'dev-test-fresh';
    const payload = {
      v: 1, tool: 'claude', status: 'idle', event: 'Stop', ts: Date.now(),
      session: sessionName, project: null, feature: null, cli_session_id: null, cwd: '/tmp',
    };
    fs.writeFileSync(path.join(statusDir, `${sessionName}.json`), JSON.stringify(payload));
    const data = svc.readStatus(sessionName);
    expect(data).not.toBeNull();
    expect(data?.status).toBe('idle');
    expect(svc.isStale(data!)).toBe(false);
  });

  test('readStatus returns stale data and isStale reports true', () => {
    const {svc, statusDir} = makeHooksService();
    ...
    const data = svc.readStatus(sessionName);
    expect(data).not.toBeNull();
    expect(svc.isStale(data!)).toBe(true);
  });
});
