...
function wt(opts: {git?: Partial<GitStatus>; pr?: Partial<PRStatus>} = {}): WorktreeInfo {
  return new WorktreeInfo({
    project: 'p',
    feature: 'f',
    path: '/p/f',
    git: new GitStatus(opts.git),
    pr: opts.pr ? new PRStatus(opts.pr) : undefined,
  });
}
...
describe('computeCodeStateChips', () => {
  ...
  test('only excluded-tracker diff present → only diff chip', () => {
    const chips = computeCodeStateChips(wt({git: {base_added_lines_excl_tracker: 12, base_deleted_lines_excl_tracker: 3}}));
  ...
  test('diff chip ignores full base counts when excl_tracker is zero', () => {
    ...
    const chips = computeCodeStateChips(wt({git: {base_added_lines: 50, base_deleted_lines: 5}}));
    expect(chips).toEqual([]);
  });

  test('only ahead → only changes chip', () => {
    const chips = computeCodeStateChips(wt({git: {ahead: 3}}));
    expect(chips).toEqual([{label: '↑3 ', color: 'cyan'}]);
  });

  test('only behind → only changes chip', () => {
    const chips = computeCodeStateChips(wt({git: {behind: 2}}));
  ...
  test('all three signals → diff, changes, PR in fixed order', () => {
    const chips = computeCodeStateChips(wt({
      git: {base_added_lines_excl_tracker: 100, base_deleted_lines_excl_tracker: 20, ahead: 1, behind: 0},
