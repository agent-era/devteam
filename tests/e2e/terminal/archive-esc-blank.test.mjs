...
test('pressing a then ESC with one worktree returns to list (not blank)', async () => {
  ...
  await waitFor(() => includesWorktree(stdout.lastFrame() || '', 'demo', 'feature-1'), {timeout: 10000, interval: 50, message: 'initial feature-1 [demo] visible'});
  let frame = stdout.lastFrame() || '';

  // Press 'v' to open archive confirmation
  stdin.emit('data', Buffer.from('v'));
  await waitForText(() => stripAnsi(stdout.lastFrame() || ''), 'Archive Feature', {timeout: 10000}).catch(async () => {
  ...
  await waitFor(() => includesWorktree(stdout.lastFrame() || '', 'demo', 'feature-1'), {timeout: 10000, interval: 50, message: 'worktree visible after ESC'});
  frame = stdout.lastFrame() || '';
