...
test('full App renders list rows with fakes', async () => {
  ...
  try {
    ...
    await waitFor(() => includesWorktree(stdout.lastFrame() || '', 'demo', 'feature-1'), {timeout: 10000, interval: 50, message: 'feature-1 [demo] visible'});
    await waitFor(() => includesWorktree(stdout.lastFrame() || '', 'demo', 'feature-2'), {timeout: 10000, interval: 50, message: 'feature-2 [demo] visible'});
  } finally {
